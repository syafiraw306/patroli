...
def _feature13_v3_text(row: Dict[str, Any]) -> str:
    return normalize_text(f"{row.get('title') or ''} ; {row.get('content') or ''}").lower()


def _feature13_v3_domain(row: Dict[str, Any]) -> str:
    try:
        parsed = urllib.parse.urlparse(str(row.get("link") or "").strip())
...
def feature13_entity_verification_v3() -> Dict[str, Any]:
    ...
    report_rows: List[Dict[str, Any]] = []
    for row in candidates:
        result = _feature13_v3_classify(row)
        report_rows.append({
            "id": row.get("id"), "action": result["action"],
            "entity": result.get("entity"), "reason": result.get("reason"),
            "evidence": result.get("evidence", []),
    ...
    with open(FEATURE13_ENTITY_VERIFY_V3_JSON, "w", encoding="utf-8") as fh:
        json.dump(payload, fh, ensure_ascii=False, indent=2)
    fieldnames = ["id", "action", "entity", "reason", "evidence", "title", "link", "published_date", "domain", "keywords", "stored_context", "content_length"]
    with open(FEATURE13_ENTITY_VERIFY_V3_CSV, "w", newline="", encoding="utf-8") as fh:
        writer = csv.DictWriter(fh, fieldnames=fieldnames)
        writer.writeheader()
        for r in report_rows:
            out = dict(r); out["evidence"] = " | ".join(r.get("evidence") or [])
    ...
    print(f"INVALID            : {counts.get('INVALID', 0)}")
    print(f"REVIEW             : {counts.get('REVIEW', 0)}")
    print(f"JSON report        : {FEATURE13_ENTITY_VERIFY_V3_JSON}")
    print(f"CSV report         : {FEATURE13_ENTITY_VERIFY_V3_CSV}")
    print("[ACTION] READ-ONLY: DATABASE TIDAK DIUBAH")
...
def main() -> None:
    ...
    if args.feature13_entity_verification_v3:
        ...
        if result.get("status") != "PASSED":
            raise RuntimeError(
                f"Entity Verification V3 gagal: {result.get('reason')}"
            )
        return
