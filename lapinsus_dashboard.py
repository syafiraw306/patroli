...
TEMPLATE_VERSION = "LAPINSUS_DELI_SERDANG_V2_5PAGES"
...
def build_lapinsus(article):
    ...
    # Jangan mengarang fakta. Bila artikel tidak cukup, tuliskan keterbatasannya.
    facts = evidence[:10] or [
        "Konten artikel tidak cukup untuk mengekstrak fakta terstruktur."
    ]

    trend = [
        "Perkembangan selanjutnya belum dapat dipastikan hanya berdasarkan satu pemberitaan dan memerlukan monitoring lanjutan.",
        "Setiap informasi baru yang mengubah situasi lokasi terkait perlu diverifikasi terhadap sumber primer atau pihak berwenang.",
    ]
    actions = [
...
def make_pdf(data):
    ...
    story += [
        ...
        _p("Bahwa informasi diperoleh dari pemberitaan media yang tercantum pada sumber artikel dan masih memerlukan verifikasi terhadap sumber primer/pihak terkait.", styles["body"]),
...
st.set_page_config(page_title="Deli Serdang Location Intelligence", layout="wide")
st.title("Deli Serdang Location Intelligence")
st.caption("DELI_SERDANG_LOCATION_KEYWORDS → Location Discovery → Heatmap → LAPINSUS")

articles = get_location_articles()
st.metric("Total artikel lokasi", len(articles))

if not articles:
    st.info("Belum ada artikel. Jalankan crawler setelah tabel deli_serdang_location_articles dan migration kolom article_images diterapkan.")
else:
    c1, c2, c3 = st.columns(3)
    with c1:
        search = st.text_input("Cari judul / keyword lokasi")
    with c2:
        status = st.selectbox("Status LAPINSUS", ["SEMUA", "PENDING", "REVIEW", "GENERATED", "FAILED", "SKIPPED"])
    with c3:
        valid = st.selectbox("Konteks lokasi", ["SEMUA", "VALID", "PERLU REVIEW"])

    filtered = []
    for article in articles:
        haystack = (_clean(article.get("title")) + " " + " ".join(article.get("matched_location_keywords") or [])).lower()
        if search and search.lower() not in haystack:
            continue
        if status != "SEMUA" and article.get("lapinsus_status") != status:
            continue
        if valid == "VALID" and not article.get("location_context_valid"):
            continue
        if valid == "PERLU REVIEW" and article.get("location_context_valid"):
            continue
    ...
    for article in filtered:
        article_id = article.get("id")
        title = _clean(article.get("title")) or "Tanpa judul"
        keywords = ", ".join(article.get("matched_location_keywords") or []) or "-"
        with st.container(border=True):
            left, right = st.columns([7, 2])
            with left:
                st.subheader(title)
                st.write(f"**Tanggal:** {_date_id(article.get('published_date'))}")
                st.write(f"**Lokasi:** {keywords}")
                st.write(f"**Konteks lokasi:** {'VALID' if article.get('location_context_valid') else 'PERLU REVIEW'}")
                st.write(f"**Sumber:** {_clean(article.get('publisher') or article.get('source')) or '-'}")
                st.write(f"**Status LAPINSUS:** {article.get('lapinsus_status') or 'PENDING'}")
                if article.get("link"):
                    st.markdown(f"[Buka artikel sumber]({article['link']})")
            with right:
                if st.button("Generate LAPINSUS PDF", key=f"lapinsus_{article_id}", use_container_width=True):
                    try:
                        data = build_lapinsus(article)
                        pdf = make_pdf(data)
                        mark_generated(article_id, pdf, data)
                        st.session_state[f"pdf_{article_id}"] = pdf
                        st.session_state[f"filename_{article_id}"] = f"LAPINSUS_{article_id}.pdf"
                        st.success("LAPINSUS 5 halaman berhasil dibuat.")
                    except Exception as exc:
                        mark_failed(article_id, exc)
                        st.error(f"Gagal membuat LAPINSUS: {type(exc).__name__}: {exc}")

                if f"pdf_{article_id}" in st.session_state:
                    st.download_button(
                        "Download LAPINSUS PDF",
                        data=st.session_state[f"pdf_{article_id}"],
                        file_name=st.session_state.get(f"filename_{article_id}", f"LAPINSUS_{article_id}.pdf"),
                        mime="application/pdf",
                        key=f"download_{article_id}",
